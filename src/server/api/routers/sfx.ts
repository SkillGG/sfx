...
export const sfxRouter = createTRPCRouter({
			...
			object({
				...
				sfx: object({
					...
					tls: array(
						object({
							...CollapsedTL.shape,
							sfx: object({ ...CollapsedOnomatopoeia.shape }),
						}),
					),
				}),
			}),
			...
			async ({
				...
				for (const tl of allTLs) {
					...
					await ctx.db.translation.update({
						where: { id: tl.id },
						data: { additionalInfo: tl.additionalInfo, updatedAt: new Date() },
					})
				}
			},
		),
	...
	createSFX: publicProcedure
		...
		.mutation(
			async ({
				...
				> & { tls: CollapsedTL[] }): CreateCreateObject => {
					return {
						text,
						def,
						info,
	...
	removeSFX: publicProcedure
		...
		.mutation(async ({ ctx, input: { id, auth } }) => {
			const loggedIn = await checkSession(ctx.db, auth)
			if (!loggedIn.ok) return loggedIn
			return await ctx.db.onomatopoeia.delete({ where: { id } })
		}),
})
