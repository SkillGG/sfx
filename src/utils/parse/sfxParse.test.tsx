...
const sF = (
	...
	return { hidden: false, index, type: 'string', value, key: key ?? `${index}` }
}
...
describe('String parse - hide', () => {
	...
	it('hide same multiple times', () => {
		...
		).toEqual({ ...emptyFieldResult, read: [hide(sF(1, 'a')), sF(2, 'b')] })
	})
...
describe('String parse - jump', () => {
	it.each([
		...
		const failObj = { ...fieldData(), [to]: `_${to}1:a;b` }
		const successObj = { ...fieldData(), [from]: `_${to}1:a`, [to]: `b` }
	...
	it('multiple jumps', () => {
		expect(
			parseSFXFields({ read: 'a;b;_d1:c', def: 'd;e;f', extra: 'g;_d2:h;i' }),
		).toEqual<SFXFieldsData>({
			...emptyFieldResult,
			read: [sF(1, 'a'), sF(2, 'b')],
			def: [
...
describe('String parse - links', () => {
	...
	it('sfx link', () => {
		...
		expect(parsed).toMatchObject<Omit<SFXFieldsData, 'data'>>({
			...emptyFieldResult,
			read: [{ type: 'sfxlink', hidden: false, ids: [1], index: 1, key: '1' }],
		})
	...
	it('multi sfx link', () => {
		...
		expect(parsed).toMatchObject<Omit<SFXFieldsData, 'data'>>({
			...emptyFieldResult,
			read: [
				{ type: 'sfxlink', hidden: false, ids: [1, 2, 3], index: 1, key: '1' },
			],
		})
	})
