 ...
 *   sfx[See also:]:2,3
 *
 *   // 4. SFX link with multiple labels
 *   sfx[See ]< for more info>:4
 *
 *   // 4a.
 *
 *   // 5. Plain string node
...
export type StringField = { type: 'string'; value: string; counter?: number }
	...
	asSFXLink(str: string, log?: Log): SFXLinkField | null {
		...
		const field: SFXLinkField = {
			...
			consume: async api => {
				...
				if (result.length > 0) {
					return result.filter(q => !!q).map(q => ({ id: q.id, label: q.text }))
				}
...
export const parseSFXFields = (
	...
	const jumpFields: {
		...
		onFail: { index: number; raw: string; key: keyof SFXFieldsData }
	}[] = []
		...
		const parseFieldData = (line: string): SFXField | null => {
			...
			if (Parser.isJump(field)) {
				if (field.to === typeKey) {
					...
					return { ...parsed, index: ++index, key: `${index}`, hidden: false }
				}
				...
				const specialField: (typeof jumpFields)[number] = {
					...
					key: field.to,
					onFail: { index: index + 1, key: typeKey, raw: field.raw },
				}
